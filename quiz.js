// ── BOOTSTRAP ─────────────────────────────────────────────────
// quiz.js loads at end of <body>; DOM is already ready.
// We decode base64 content first, then run all init functions.

var _initQueue = [];
function registerInit(fn) {
  // If decode already done, run immediately; otherwise queue
  if (window._decoded) { try { fn(); } catch(e) {} }
  else { _initQueue.push(fn); }
}

function decodeAll() {
  Array.from(document.querySelectorAll('[data-c]')).forEach(function(el) {
    try {
      var tmp = document.createElement('div');
      tmp.innerHTML = atob(el.getAttribute('data-c'));
      var node = tmp.firstElementChild || tmp.firstChild;
      if (node && el.parentNode) el.parentNode.replaceChild(node, el);
    } catch(e) { console.warn('decode error:', e); }
  });
  window._decoded = true;
  _initQueue.forEach(function(fn){ try { fn(); } catch(e) { console.warn(e); } });
  _initQueue = [];
}

// Run immediately — DOM is ready when this script executes
decodeAll();

// Global click handlers (survive decode)
document.addEventListener('click', function(e) {
  // Click-to-speak
  var t = e.target.closest('.vthai,.cc-thai,.thai-cell');
  if (t) { speak((t.textContent || '').trim()); return; }

  // Mobile tooltip (.conj / .vword)
  var tip = e.target.closest('.conj,.vword');
  if (tip) {
    var on = tip.classList.contains('tip-on');
